...
export default function LabResults({ params: { locale } }: { params: { locale: string } }) {
  ...
  async function submitResults() {
    ...
    try {
      ...
      setMsg('✅ Results saved. Pending validation.');
      setStep('validate');
    } catch (err: any) { setMsg(`❌ ${err.message}`); }
    setSaving(false);
  }
    ...
    <DashboardShell navItems={nav} title="Enter Lab Results" locale={locale}>
      <div style={{ maxWidth: 700 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #e8e8e8', padding: '20px 22px', marginBottom: 16 }}>
          <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>Step 1 — Scan or enter barcode</div>
          <div style={{ display: 'flex', gap: 10 }}>
            <input value={barcode} onChange={e => setBarcode(e.target.value)} onKeyDown={e => e.key === 'Enter' && receiveSample()} placeholder="e.g. LAB-20240115-0001" style={{ flex: 1 }} />
            <button onClick={receiveSample} style={{ flexShrink: 0 }}>Receive sample</button>
          </div>
        </div>
        ...
        {sample && (
          <div style={{ background: '#f0f7ff', borderRadius: 10, border: '1px solid #cce3f9', padding: '14px 18px', marginBottom: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#185FA5', marginBottom: 6 }}>Sample: {sample.barcode}</div>
            <div style={{ fontSize: 13, color: '#555' }}>
              Patient: <strong>{sample.patient_name}</strong> ({sample.patient_mrn}) ·
              Test: <strong>{sample.test_name}</strong> ·
        ...
        {step === 'enter' && sample && (
          <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #e8e8e8', padding: '20px 22px', marginBottom: 16 }}>
            <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 14 }}>Step 2 — Enter results</div>

            {components.map((c, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr auto', gap: 8, marginBottom: 8, alignItems: 'center' }}>
                <input value={c.componentName} onChange={e => setComponents(p => p.map((r,idx) => idx===i ? {...r,componentName:e.target.value} : r))} placeholder="Component (e.g. Haemoglobin)" />
                <input value={c.resultValue}   onChange={e => setComponents(p => p.map((r,idx) => idx===i ? {...r,resultValue:e.target.value}   : r))} placeholder="Result (e.g. 13.5)" />
                <input value={c.unit}          onChange={e => setComponents(p => p.map((r,idx) => idx===i ? {...r,unit:e.target.value}          : r))} placeholder="Unit (e.g. g/dL)" />
                <button type="button" onClick={() => removeRow(i)} style={{ background: '#fef2f2', color: '#991b1b', padding: '8px 10px', fontSize: 16 }}>×</button>
              </div>
            ))}

            <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
              <button type="button" onClick={addRow} style={{ background: '#f0f0f0', color: '#333' }}>+ Add component</button>
              <button onClick={submitResults} disabled={saving}>{saving ? 'Saving…' : 'Submit results'}</button>
            </div>
          </div>
        )}
        ...
        {step === 'validate' && (
          <div style={{ background: '#f0fdf4', borderRadius: 10, border: '1px solid #86efac', padding: '20px 22px' }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: '#166534', marginBottom: 6 }}>Results submitted</div>
            <div style={{ fontSize: 13, color: '#15803d' }}>Results are pending validation by a supervisor. The ordering doctor will be notified when validated.</div>
