...
export default function BookAppointment({ params: { locale } }: { params: { locale: string } }) {
  ...
  async function handleBook(e: React.FormEvent) {
    ...
    try {
      ...
      setMsg(`✅ ${t('Appointment booked successfully')}`);
  ...
  return (
    <DashboardShell navItems={nav} title={t('nav.book')} locale={locale}>
        ...
        <form onSubmit={handleBook}>
          <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #e8e8e8', padding: '22px 24px', marginBottom: 16 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 500, marginBottom: 6 }}>{t('Patient *')}</label>
              <input value={patSearch} onChange={e => setPatSearch(e.target.value)} placeholder={t('Search patient by name, MRN, or phone…')} />
              {patients.length > 0 && (
            ...
            {/* Doctor */}
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 500, marginBottom: 6 }}>{t('Doctor *')}</label>
            ...
            {/* Appointment type */}
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 500, marginBottom: 6 }}>{t('Appointment type *')}</label>
            ...
            {/* Date */}
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 500, marginBottom: 6 }}>{t('Date *')}</label>
            ...
            {slots.length > 0 && (
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 500, marginBottom: 8 }}>{t('Available slots')}</label>
            ...
            {/* Notes */}
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 500, marginBottom: 6 }}>{t('dash.notes')}</label>
              <textarea value={form.notes} onChange={e => set('notes', e.target.value)} rows={3} placeholder={t('Optional notes…')} />
            </div>
